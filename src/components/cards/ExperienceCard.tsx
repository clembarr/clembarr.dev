 ...
 * @description Tilt-on-hover experience card used in the career timeline.
 * @param entry - Experience data to display
 * @param additionalStyles - Inline styles forwarded to the root element (position, z-index, etc.)
 */
const ExperienceCard = ({ entry, additionalStyles }: ExperienceCardProps) => {
    ...
    <div
        ref={cardRef}
        id={`card-${entry.title[currentLang]}-container`}
        className={`
        group
        ${styles.sizeFull}
    ...
    >
        <div id={`top-card-${entry.title[currentLang]}-line`}
        className={`
            absolute
