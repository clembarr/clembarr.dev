...
**La matière première est le langage.** Le hero sombre est une chouette effraie tramée en
pixels menthe, sans lueur de couleur. Ce n'est pas la même image recolorée que le clair :
...
| `glitch-slice-1`, `glitch-slice-2`, `glitch-flicker` — les rafales de glitch **monochrome** qui couvraient la figure du fil d'actualité en sombre | `index.css:310` à `380` |
| `card-top-to-bottom` et ses variantes — le jeu de cartes d'un slider de projets qui n'existe plus | `index.css` |
| `GALAXY_CLUSTER_COLORS` — six couleurs pour une vue « galaxy » inexistante | `uiConstants.ts:1179` |

**21 des 31 keyframes sont morts, et ce sont presque tous les maximalistes.**
...
monochrome, et a noté pourquoi — avant d'éteindre aussi celle-là.
...
- une carte arrondie au-delà de `rounded-md` (8 px), un média de carte à fond perdu, un
  texte de carte collé au bord — voir « Le gabarit de carte » ; `full` seulement pour les
  tags et pastilles.

## Le gabarit de carte — non négociable

Une carte de ce site a **une** forme, celle de la page projets (`ProjectPreview.tsx`). Toute
carte neuve ou retouchée la reprend, mesure pour mesure. C'est la convention la plus visible
du site et la plus facile à perdre : chaque écart pris seul semble anodin, leur somme fait
un site générique.

| Mesure | Valeur | Référence |
|---|---|---|
| Arrondi | `rounded-md` — 8 px. Ni `lg`, ni `xl` | `ProjectPreview.tsx:94` |
| Média | **encarté** : `m-[6%] mb-0` — 6 % de la largeur en haut et sur les côtés, jamais à fond perdu | `ProjectPreview.tsx:112` |
| Texte | `px-[8%]` — en retrait de 2 % sur le média | `ProjectPreview.tsx:124` |
| Survol | hairline d'accent en haut, voir « Le vocabulaire » | |

Les pourcentages suivent la largeur de la carte : une carte étroite garde les mêmes
proportions, pas les mêmes pixels. S'y conforment : la page projets, les cartes d'actualité
(`News.tsx:143`, `157`, `208`), les cartes de parcours et le détail des certifications
(`CareerTimeline.tsx:409`, `684`).

**Vérifier en mesurant, pas en lisant les classes.** `index.css` pose un `padding` sur tout
`<button>`, hors couche : il écrase un `p-0` sans rien dire, et une carte cliquable en
`<button>` gagnait ainsi 9 px de marge fantôme. Relever `getBoundingClientRect()` du média
et du titre par rapport à la carte, et comparer aux 6 % et 8 %.

Écarts connus, non tranchés : les cartes de blog (`BlogCard.tsx`, `rounded-lg` et média à
fond perdu), les widgets « à propos » (`About.tsx`, `rounded-[5px]`). `ProjectCard.tsx`
(`rounded-xl`) n'est monté nulle part.
...
- **Une date est du texte courant** : Montserrat, graisse normale, atténuée par l'opacité.
  Jamais en gras, jamais en `font-mono` — cartes de blog, d'actualité et de parcours
  suivent la même règle. Seules les années graduant l'axe de la frise sont en `font-mono`,
  comme des repères d'échelle.
